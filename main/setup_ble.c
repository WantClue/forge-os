...
#define SETUP_BLE_DEVICE_NAME_PREFIX "BitForge-"
...
{
    char *value = nvs_config_get_string(NVS_CONFIG_WIFI_SSID, WIFI_SSID);
    setup_ble_copy_string(setup_ble_state.wifi_ssid, sizeof(setup_ble_state.wifi_ssid), value);
    free(value);

    setup_ble_state.wifi_password[0] = '\0';

    value = nvs_config_get_string(NVS_CONFIG_STRATUM_URL, CONFIG_STRATUM_URL);
    ...
    value = nvs_config_get_string(NVS_CONFIG_STRATUM_USER, CONFIG_STRATUM_USER);
