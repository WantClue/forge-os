...
esp_err_t coinbase_process_notification(const mining_notify *notification,
    ...
    if (user_address) {
        if (strncmp(user_address, "bcrt1", 5) == 0) {
            bech32_hrp = "bcrt";
            is_testnet = true;
    ...
    // 1. Parse Coinbase 1 for ScriptSig info
    ...
    // 2. Parse Coinbase 2 for Outputs
    ...
    // Skip nSequence (4 bytes)
    if (offset + 4 > coinbase_2_len) {
        free(coinbase_2_bin);
        return ESP_ERR_INVALID_ARG; // No room for outputs
    }
    offset += 4;
    ...
    for (uint64_t i = 0; i < num_outputs && offset < coinbase_2_len; i++) {
        ...
        if (result->output_count < MAX_COINBASE_TX_OUTPUTS) {
            ...
            coinbase_decode_address_from_scriptpubkey(coinbase_2_bin + offset, script_len,
                                                      out->address, MAX_ADDRESS_STRING_LEN,
                                                      bech32_hrp, is_testnet);
            out->value_satoshis = value_satoshis;
            result->output_count++;
        }
